...
class EstimatesInvoicesController extends Controller
{
    ...
    public function list()
    {
        ...
        $estimates_invoices = $estimates_invoices->orderBy($sort, $order)
            ...
            if ($canEdit) {
                $actions .= '<a href="' . route('estimates-invoices.edit', ['id' => $estimates_invoice->id]) . '" title="' . get_label('update', 'Update') . '"><i class="bx bx-edit mx-1"></i></a>';
            }
            ...
            $actions .= '<a href="' . route('estimates-invoice.pdf', ['id' => $estimates_invoice->id]) . '" title="PDF" target="_blank">' .
            '<i class="bx bxs-file-pdf text-secondary mx-2"></i>' .
            '</a>';

            // dd($estimates_invoice);
                return [
    ...
    public function pdf(Request $request, $id)
    {
        ...
        $admin_settings = json_decode(Admin::find(getAdminIdByUserRole())->admin_settings, true);

        // Check for logo
        if (isset($admin_settings['full_logo']) && !empty($admin_settings['full_logo'])) {
            $logo = 'storage/' . $admin_settings['full_logo'];
        } elseif (isset($general_settings['full_logo']) && !empty($general_settings['full_logo'])) {
            $logo = 'storage/' . $general_settings['full_logo'];
        } else {
            $logo = 'storage/logos/default_full_logo.png';
        }

        // Check for company title
        if (isset($admin_settings['company_title']) && !empty($admin_settings['company_title'])) {
            $company_title = $admin_settings['company_title'];
